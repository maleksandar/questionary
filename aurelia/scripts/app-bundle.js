...
define('resources/elements/question-list',['exports', 'aurelia-framework', 'aurelia-fetch-client'], function (exports, _aureliaFramework, _aureliaFetchClient) {
  ...
  var QuestionList = exports.QuestionList = (_dec = (0, _aureliaFramework.inject)(_aureliaFetchClient.HttpClient), _dec(_class = (_class2 = function () {
    function QuestionList(httpClient) {
      ...
      this.httpClient.fetch('questions?include=Answers&include=Tags&offset=0').then(function (response) {
        return response.json();
      }).then(function (responseObj) {
        if (responseObj.count != 0) {
          _this.questions = responseObj.rows;
          _this.questionsIsNotEmpty = true;
          _this.currentIndex = 0;
          var count = responseObj.count;
          if (count % 3 == 0) _this.pageIndexes = Array.from(new Array(Math.floor(count / 3)), function (x, i) {
            return i;
          });else _this.pageIndexes = Array.from(new Array(Math.floor(count / 3) + 1), function (x, i) {
            return i;
          });
          _this.paginationLower = _this.questionsIsNotEmpty && _this.pageIndexes.length <= 5;
          _this.paginationGreater = _this.questionsIsNotEmpty && _this.pageIndexes.length > 5;
          _this.middlePages = _this.getPages(_this.currentIndex);
        }
      });
    }
    ...
    QuestionList.prototype.setPage = function setPage(index) {
      ...
      this.httpClient.fetch('questions?include=Answers&include=Tags&offset=' + (index * 3).toString()).then(function (response) {
        return response.json();
      }).then(function (responseObj) {
        _this2.questions = responseObj.rows;
        _this2.questionsIsNotEmpty = true;
        var count = responseObj.count;
        if (count % 3 == 0) _this2.pageIndexes = Array.from(new Array(Math.floor(count / 3)), function (x, i) {
          return i;
        });else _this2.pageIndexes = Array.from(new Array(Math.floor(count / 3) + 1), function (x, i) {
          return i;
        });
        _this2.paginationLower = _this2.questionsIsNotEmpty && _this2.pageIndexes.length <= 5;
        _this2.paginationGreater = _this2.questionsIsNotEmpty && _this2.pageIndexes.length > 5;
        _this2.middlePages = _this2.getPages(index);
      });
  ...
  var Question = exports.Question = (_dec = (0, _aureliaFramework.inject)(_aureliaFetchClient.HttpClient, toastr, _auth.Auth, _aureliaDialog.DialogService, _aureliaEventAggregator.EventAggregator), _dec2 = (0, _aureliaFramework.computedFrom)('auth.currentUser.userId', 'content.createdByUserId'), _dec3 = (0, _aureliaFramework.computedFrom)('auth.isLogedIn'), _dec(_class = (_class2 = function () {
    ...
    Question.prototype.quickAnswer = function quickAnswer() {
      ...
      this.httpClient.fetch('answers', { method: 'post', body: (0, _aureliaFetchClient.json)({ text: this.answerText, QuestionId: this.content._id }) }).then(function () {
        _this.answerText = "";
        _this.content.Answers.push({});
        _this.ea.publish('questionAnswered', { id: _this.content._id });
    ...
    Question.prototype.delete = function _delete() {
      ...
      this.dialogService.open({ viewModel: _confirmationDialog.ConfirmationDialog, model: { headline: "Delete question", message: "Are you sure you want to delete this question?" } }).then(function (response) {
        if (!response.wasCancelled) {
          ...
          _this2.httpClient.fetch('questions/' + _this2.content._id, { method: 'delete' }).then(function () {
            _this2.toastr.success('You have successfully deleted your question');
            _this2.deleted = true;
          });
        }
      });
